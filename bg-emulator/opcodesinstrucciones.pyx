from cython cimport uchar
from GameBoy import cpu  # Asegúrate de que "cpu" esté importado correctamente

cdef uchar SET_1F4(cpu.CPU) noexcept nogil:  # 1F4 SET 6,H
...
cdef uchar SET_1F5(cpu.CPU) noexcept nogil:  # 1F5 SET 6,L
...
cdef uchar SET_1F6(cpu.CPU) noexcept nogil:  # 1F6 SET 6,(HL)
...
cdef uchar SET_1F7(cpu.CPU) noexcept nogil:  # 1F7 SET 6,A
...
cdef uchar SET_1F8(cpu.CPU) noexcept nogil:  # 1F8 SET 7,B
...
cdef uchar SET_1F9(cpu.CPU) noexcept nogil:  # 1F9 SET 7,C
...
cdef uchar SET_1FA(cpu.CPU) noexcept nogil:  # 1FA SET 7,D
...
cdef uchar SET_1FB(cpu.CPU) noexcept nogil:  # 1FB SET 7,E
...
cdef uchar SET_1FC(cpu.CPU) noexcept nogil:  # 1FC SET 7,H
...
cdef uchar SET_1FD(cpu.CPU) noexcept nogil:  # 1FD SET 7,L
...
cdef uchar SET_1FE(cpu.CPU) noexcept nogil:  # 1FE SET 7,(HL)
...
cdef uchar SET_1FF(cpu.CPU) noexcept nogil:  # 1FF SET 7,A
